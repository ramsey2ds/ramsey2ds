...
class ContactForm {
    ...
    async handleSubmit(e) {
        ...
        // Vérifier le cooldown avant l'envoi
        const lastSubmission = localStorage.getItem(this.STORAGE_KEY);
        if (lastSubmission) {
            const lastTime = parseInt(lastSubmission);
            const now = Date.now();
        ...
        try {
            ...
            if (data.success) {
                ...
                // Désactiver le formulaire pour 48h
                this.checkCooldown();
            } else {
...
// Mentions Légales Modal handler
class MentionsModal {
    constructor() {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Add scroll data attributes to specific elements
    document.querySelectorAll('.about-content, .expertise-card, .contact-info, .contact-form-wrapper').forEach(el => {
        el.setAttribute('data-scroll', '');
    });
    
    // Re-initialize scroll animations after adding attributes
    new ScrollAnimations();
    
    // Gérer les liens supplémentaires du footer
